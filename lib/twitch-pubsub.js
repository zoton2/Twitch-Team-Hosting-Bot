// STUFF TO IMPROVE:
// If the first attempt at connecting fails, the callback in initializeConnection will not fire.
// Add jitter.
...
function initializeConnection(connectionNumber, callback) {
	...
	ws[connectionNumber].on('error', function(error) {
		...
		refreshConnection(connectionNumber);  // Not sure if 'close' is also fired on error, here for safety.
	});
		...
		if (topicType === 'video-playback') {
			switch(messageParse.type) {
				...
				case 'stream-down':
					...
					loggingFuncs.logMessage(null, 'stream-down event received (' + channelName + ')');  // debug
					break;
				case 'viewcount':
