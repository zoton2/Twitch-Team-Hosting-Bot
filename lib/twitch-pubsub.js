...
function initializeConnection(connectionNumber, callback) {
	...
	ws[connectionNumber].on('open', function() {
		loggingFuncs.logMessage(null, 'Connected to PubSub connection (' + connectionCount + ').');
	...
	ws[connectionNumber].on('close', function() {
		loggingFuncs.logMessage(null, 'Disconnected from PubSub connection (' + connectionCount + ').');
...
function handleMessages(connectionNumber, data) {
	...
	if (parsed.type === 'RECONNECT') {
		loggingFuncs.logMessage(null, 'A RECONNECT has been issued on PubSub connection (' + connectionCount + ').');
		refreshConnection(connectionNumber);
	}
...
function pingConnection(connectionNumber) {
	var pingCheckTimeout = setTimeout(function() {
		loggingFuncs.logMessage(null, 'Ping timeout on PubSub connection (' + connectionCount + ').');
		refreshConnection(connectionNumber);
	}, 10000);
...
function refreshConnection(connectionNumber) {
	...
	if (wsOpen[connectionNumber]) {
		loggingFuncs.logMessage(null, 'Refreshing PubSub connection (' + connectionCount + ').');
