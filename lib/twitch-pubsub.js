...
// NOT SURE IF THIS FUNCITON IS REALLY NEEDED AS IT'S PRETTY MUCH WHAT listenToTopics DOES.
		...
		if (topicType === 'video-playback') {
			switch(messageParse['type']) {
				...
				case 'viewcount':
					...
					globalVars.twitchPubSubEvents.emit('viewCount', channelName, parseInt(messageParse['viewers']), messageParse['server_time']);
					break;
			}
		}
	}
