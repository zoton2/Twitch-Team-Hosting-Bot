...
// No error checking yet (when the server sends an error).
...
// Used to disconnect all PubSubs on close for a clean disconnection.
exports.disconnectAll = function() {
	appClosing = true;
	for (var i = 0; i < listOfConnections.length; i++) {
		ws[listOfConnections[i]].close(1000, '', {keepClosed: true});
	}
}
...
function createNewConnection(topics, callback) {
	...
	logging.logMessage(null, 'Creating new PubSub connection (' + connectionCount + ').');
...
function initializeConnection(connectionNumber, callback) {
	ws[connectionNumber] = new WebSocket(address, null, {constructor: Html5WebSocket});
	
	ws[connectionNumber].onerror = function(error) {
		logging.logMessage(null, 'Error on PubSub connection (' + connectionNumber + ') [' + error.code + '].');
		ws[connectionNumber].close();
	}
	...
	ws[connectionNumber].onopen = function() {
		logging.logMessage(null, 'Connected to PubSub connection (' + connectionNumber + ').');
		exports.events.emit('connected', connectionNumber);
	...
	ws[connectionNumber].onclose = function() {
		if (wsOpen[connectionNumber]) {logging.logMessage(null, 'Disconnected from PubSub connection (' + connectionNumber + ').');}
		if (!appClosing) {refreshConnection(connectionNumber);}
	}
...
function handleMessages(connectionNumber, data) {
	...
	if (parsed.type === 'RECONNECT') {
		logging.logMessage(null, 'A RECONNECT has been issued on PubSub connection (' + connectionNumber + ').');
		...
		if (topicType === 'video-playback') {
			switch(messageParse.type) {
				case 'stream-up':
					...
					exports.events.emit('streamUp', channelName, messageParse.play_delay, messageParse.server_time);
					break;
				case 'stream-down':
					// emits: channel name, server time
					exports.events.emit('streamDown', channelName, messageParse.server_time);
					break;
				case 'viewcount':
					// emits: channel name, viewers, server time
					exports.events.emit('viewCount', channelName, messageParse.viewers, messageParse.server_time);
					break;
			}
		}
	}
...
function pingConnection(connectionNumber) {
	var pingCheckTimeout = setTimeout(function() {
		logging.logMessage(null, 'Ping timeout on PubSub connection (' + connectionNumber + ').');
		refreshConnection(connectionNumber, true);
	}, 10000);
...
function refreshConnection(connectionNumber, close) {
	if (wsOpen[connectionNumber]) {
		logging.logMessage(null, 'Refreshing PubSub connection (' + connectionNumber + ').');
		wsOpen[connectionNumber] = false;
		exports.events.emit('disconnected', connectionNumber);
	}
