...
function listenForCommands(team, message, admin, username, whisper, callback) {
	...
	if (message.toLowerCase() === '!hostedtime' || message.toLowerCase() === '!hostedchannel') {
		...
		if (globalVars.currentHostedChannel[team]) {
			...
			return callback('We have been hosting ' + globalVars.currentHostedChannel[team].displayName + ' for ' + hostingFuncs.calculateHostedTime(team) + ' (hosting length: ' + hostingFuncs.formatMS(globalVars.currentHostRefreshLength[team]) + ').');
		}
