...
function listenForCommands(team, message, admin, username, whisper, callback) {
	...
	if (message.toLowerCase() === '!hostbotcheck') {
		...
		sentCallback = true; commandUsed = true;
	...
	// If this message actually tried to use a command, logs who tried to use the command and the message they sent.
	if (commandUsed) {loggingFuncs.logMessage(team, username + ' has used a command' + (whisper?' (via whisper)':'') + ': ' + message);}
