...
exports.setUp = function() {
	logging.logMessage(null, 'The web server is now being set up.');
	...
	server.listen(globalVars.settings.serverPort, function() {
		logging.logMessage(null, 'The web server has been set up and is now listening for requests.');
	});
			...
			if (globalVars.channels.hasOwnProperty(team)) {
				...
				if (globalVars.active[team]) {
					...
					if (globalVars.currentHostedChannel[team]) {
						...
						' (' + hosting.calculateHostedTime(team) + '/' +
						hosting.formatMS(globalVars.currentHostRefreshLength[team]) + ')';
					}
...
function handleRequest(request, response) {
	try {
		...
		if (globalVars.settings.logServerAccess) {
			logging.logMessage(null, 'Someone has tried to access the server with this request URL: ' + request['url']);
		}
	} catch (exception) {
...
function setUpAPI() {
	dispatcher.onGet("/api/statistics", function(req, res) {
		...
		res.end(JSON.stringify(globalVars.statistics));
	});
}