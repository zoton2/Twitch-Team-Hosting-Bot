...
globalVars.twitchPubSubEvents.on('streamUp', function(channel) {
	...
	loggingFuncs.logMessage(null, channel + ' went online.');
});

globalVars.twitchPubSubEvents.on('streamDown', function(channel) {
	removeOfflineChannel(channel);
	loggingFuncs.logMessage(null, channel + ' went offline.');
});
...
globalVars.twitchPubSubEvents.on('viewCount', function(channel, viewers) {
	...
	if (!globalVars.liveStreamsInfo[channel]) {
		...
		loggingFuncs.logMessage(null, channel + ' was already online (' + viewers + ' viewers) but we didn\'t know.');
	}
...
function updateChannelDetails(channel) {
	loggingFuncs.logMessage(null, channel + ' is having their details checked/updated.');
	
	twitchAPI.getChannelData(channel, function(error, errorType, response) {
		// extra check because sometimes this ends up running once a channel goes offline(? still testing)
		if (!error/* && globalVars.liveStreamsInfo[channel]*/) {
			var currentChannelDetails = {
				displayName: response['display_name'] || response.name,
				status: response.status,
				game: response.game,
				viewers: globalVars.liveStreamsInfo[channel].viewers
			};
			...
			if (currentChannelDetails.status !== globalVars.liveStreamsInfo[channel].status
				|| currentChannelDetails.game !== globalVars.liveStreamsInfo[channel].game) {
					...
					loggingFuncs.logMessage(null, channel + ' updated their game or title.');
				}
