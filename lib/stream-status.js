...
exports.setUpHostChecking = function(team) {
	...
	globalVars.client[team].on('hosting', function(channel, target, viewers) {
		...
		if (!globalVars.currentHostedChannel[team]
			...
			(target.toLowerCase() !== globalVars.currentHostedChannel[team].username
			|| target !== globalVars.currentHostedChannel[team].displayName))) {
			...
			loggingFuncs.logMessage(team, channel + ' is hosting ' + target + ', we will not host on their channel for 25 minutes.');
			...
			timeouts[channel] = setTimeout(function() {
				...
				loggingFuncs.logMessage(team, channel + ' can now be hosted on again.');
			}, 1500000);
		}
	});
}
