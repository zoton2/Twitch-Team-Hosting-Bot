	...
	twitchPubSub.listenToTopics(processVideoPlaybackTopics(newChannels), function() {
		...
		async.whilst(
			...
			function(err) {
				...
				loggingFuncs.logMessage(null, 'These channels are now being listened to: ' + newChannels.toString());
...
exports.unlistenToChannels = function(channels, callback) {
	...
	twitchPubSub.unlistenToTopics(processVideoPlaybackTopics(oldChannels), function() {
		...
		loggingFuncs.logMessage(null, 'These channels are no longer being listened to: ' + oldChannels.toString());
		if (callback) {callback();}
	});
}
