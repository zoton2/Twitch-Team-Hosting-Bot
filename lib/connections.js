			...
			function(callback) {
				twitchAPI.getTeamChannels(loginDetails[i].team, function(error, errorType, response) {
					if (!error) {
						...
						teamChannels[loginDetails[i].team] = [];
						
						// Pushes all of the channels on the team into the correct array.
						for (var j = 0; j < response.channels.length; j++) {
							teamChannels[loginDetails[i].team].push(response.channels[j].channel.name);
						}
...
function connectToChat(team, account, oauth, callback) {
	...
	// Sets up the client.
	exports.client[account] = new irc.client(ircOptions);
	exports.client[account].connect();
	
	// Joins the team channels on connect (which will also happen if we have to reconnect).
	exports.client[account].on('connected', function(address, port) {
		for (var i = 0; i < teamChannels[team].length; i++) {
			exports.client[account].join(teamChannels[team][i]);
		}
	});
	
	// Calls back once connected (but only once!)
	exports.client[account].once('connected', function(address, port) {
		callback();
	});
}