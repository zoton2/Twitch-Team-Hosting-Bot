...
exports.manuallyHostChannel = function(team, channel, length, callback) {
	checkIfChannelExistsAndOnline(channel, function(exists, name, game) {
		if (!exists) {callback(false, 'That channel doesn\'t exist or is offline.');}
			...
			loggingFuncs.logMessage(team, 'A manual host has been triggered for ' + name + (lengthInMS?' with a manual hosting length of '+length+'m.':'.'));
...
exports.chooseChannel = function(team, channel, manualHostLength, preferredGame) {
	...
	async.waterfall([
		...
		if (chosenChannel) {
			...
			globalVars.currentRefreshLength[team] = refreshLength;
			...
			loggingFuncs.logMessage(team, 'Going to host ' + chosenChannel + '; taking 2 minutes to check what channels are offline before we host on them.');
			
			// This function will not trigger the callback for 2 minutes (see the notes on the function below).
			// During this time the target channel might go offline, but they should be unhosted 5 minutes later if so.
			getOfflineChannels(otherTeamChannels, function(offlineChannels) {
...
// Checks thrice with a delay of 1 minute because Twitch's API can sometimes return incorrect results.
		...
		function(callback) {
			twitchAPI.getStreamStatus(channels, function(error, errorType, response) {
				if (!error) {
					...
					// Only calls back with a delay if this is the first or second check.
					if (i < 2) {setTimeout(callback, 60000);}
					else {callback();}
					i++;
				}
			});
...
function checkIfOffline(team) {
	...
	var timeout = setInterval(function() {
		twitchAPI.getStreamStatus([globalVars.currentHostedChannel[team].toLowerCase()], function(error, errorType, response) {
			...
			if (globalVars.adminCommandsActive[team] && !error && !detectedOffline && response.streams.length === 0) {
				var justHostedChannel = globalVars.currentHostedChannel[team];
