	...
	async.waterfall([
		...
		if (chosenChannel) {
			...
			getOfflineChannels(team, otherTeamChannels, function(offlineChannels) {
				...
				checkIfOffline(team);
				statsFuncs.incrementChannelStat(team, chosenChannel.username);
				...
				if (offlineChannels.length > 0) {
					...
					loggingFuncs.logMessage(team, 'Channel(s) we will attempt to host on: ' + offlineChannels.join(', '));
					...
					loggingFuncs.logMessage(team, 'All channels currently online or do not want to be hosted on so ' + chosenChannel.displayName + ' has not been hosted anywhere.');
					globalVars.adminCommandsActive[team] = true;
				}
			});
		}
