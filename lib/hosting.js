	...
	exports.getOnlineChannels(team, function(onlineChannels) {
		...
		if (onlineChannels.length > 0) {
			...
			// More than 1...
				...
				for (var i = 0; i < onlineChannels.length; i++) {
					for (var j = 0; j < globalVars.teamPreferredGames[team].length; j++) {
						if (onlineChannels[i].currentGame.toLowerCase().indexOf(globalVars.teamPreferredGames[team][j].toLowerCase()) >= 0) {
		...
		if (chosenChannel) {
			...
			globalVars.teamTimeouts[team] = setTimeout(function() {exports.chooseChannel(team);}, globalVars.recheckLength)
		}
	});
}
	...
	twitchAPI.getTeamLiveChannels(team, function(error, errorType, response) {
		if (!error) {
			...
			for (var i = 0; i < response.channels.length; i++) {
				onlineChannels.push({
					username: response.channels[i].channel.name,
