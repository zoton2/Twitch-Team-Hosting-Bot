...
exports.manuallyHostChannel = function(team, channel, length, callback) {
	checkIfChannelExistsAndOnline(channel, function(exists, name, game) {
		if (!exists) {callback(false, 'That channel doesn\'t exist or is offline.');}
			...
			var preferredGame = (game) ? checkIfPlayingPreferredGame(team, game) : false;
			loggingFuncs.logMessage(team, 'A manual host has been triggered for ' + name + (length?' with a manual hosting length of '+length+'m.':'.'));
			loggingFuncs.logMessage(team, name + ' is ' + (preferredGame?'playing a preferred game.':'not playing a preferred game.'));
...
exports.chooseChannel = function(team, channel, manualHostLength, preferredGame) {
	...
	preferredGame = (preferredGame) ? preferredGame : true;
					...
					if (team) {
						...
						if (onlineChannels.length > 0) {
							...
							if (onlineChannels.length === 1) {
								...
								loggingFuncs.logMessage(team, 'Only 1 channel online ' + (globalVars.preferredGames[team].length > 0)?(preferredGame?'(playing a preferred game)':'(not playing a preferred game)'):'' + ', will host them.');
							}
...
// Checks if the game name passed to it is playing a preferred game (for that team) or not.
