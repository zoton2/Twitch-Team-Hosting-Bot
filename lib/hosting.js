...
exports.manuallyHostChannel = function(team, channel, length, callback) {
	checkIfChannelExistsAndOnline(channel, function(exists, username, displayName, game, viewers) {
		if (!exists) {callback(false, 'That channel doesn\'t exist or is offline.');}
			...
			// If the channel is not on the team, sets up listening for them temporarily.
			// This is needed so we can identify when they go offline.
			if (globalVars.allTeamChannels[team].indexOf(channel) < 0) {streamStatus.listenToChannels([channel]);}
...
function clearVariables(team) {
	// If the channel we are currently hosting is not on the team (if someone is being hosted), unlistens to them.
	if (globalVars.currentHostedChannel[team] && globalVars.allTeamChannels[team].indexOf(globalVars.currentHostedChannel[team].username) < 0) {
