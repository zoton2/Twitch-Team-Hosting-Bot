		...
		function(callback) {
			...
			if (!chosenChannel) {
				...
				for (var i = 0; i < onlineChannels.length; i++) {
					if (onlineChannels[i].username === globalVars.currentHostedChannel[team].username || onlineChannels[i].username === globalVars.lastHostedChannel[team].username) {
			...
			if (offlineChannels.length > 0) {
				...
				globalVars.client[team].on('hosting', hostEvent = function(channel, target, viewers) {
					...
					if (channelsChecked.indexOf(channel) < 0 && offlineChannels.indexOf(channel) && target.toLowerCase() === chosenChannel.username) {
						...
						checkIfChannelFullyHosted(team, channel, chosenChannel.username, hostEvent, noticeEvent, channelsChecked, offlineChannels, totalViewersForHost);
					}
				});
...
function checkIfOffline(team) {
	...
	globalVars.client[team].on('notice', globalVars.offlineNotice[team] = function(channel, msgid, message) {
		...
		if (globalVars.adminCommandsActive[team] && !detectedOffline && msgid === 'host_target_went_offline'
			&& message.toLowerCase().indexOf(globalVars.currentHostedChannel[team].username) === 0 /* might not work for new display names */) {
	...
	var timeout = setInterval(function() {
		twitchAPI.getStreamStatus([globalVars.currentHostedChannel[team].username], function(error, errorType, response) {
