...
exports.chooseChannel = function(team, channel, manualHostLength, preferredGame) {
	...
	async.waterfall([
		...
		if (chosenChannel) {
			...
			loggingFuncs.logMessage(team, 'Going to host ' + chosenChannel + '.');
	...
	if (!globalVars.teamChannels[team]) {
		...
		twitchAPI.getTeamChannels(team, function(error, errorType, response) {
			...
			if (!error) {
				for (var i = 0; i < response.length; i++) {
					if (globalVars.liveStreamsInfo[response[i].username]) {
						onlineChannels.push({
							username: globalVars.liveStreamsInfo[response[i].username].displayName,
							currentGame: globalVars.liveStreamsInfo[response[i].username].game,
							currentViewers: globalVars.liveStreamsInfo[response[i].username].viewers
						});
					}
				}
			}
				...
				loggingFuncs.logMessage(team, 'Error accessing the Twitch API for "getTeamChannels".');
			}
		...
		for (var i = 0; i < globalVars.teamChannels[team].length; i++) {
			if (globalVars.liveStreamsInfo[globalVars.teamChannels[team][i]]) {
				onlineChannels.push({
					username: globalVars.liveStreamsInfo[globalVars.teamChannels[team][i]].displayName,
