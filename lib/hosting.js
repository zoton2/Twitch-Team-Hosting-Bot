		...
		if (globalVars.currentHostedChannel[team]) {
			...
			globalVars.client[team].on('notice', noticeEvent = function(channel, msgid, message) {
				if (channelsChecked.indexOf(channel) < 0) {
					...
					if (msgid === 'not_hosting' || msgid === 'no_permission' || msgid === 'msg_banned') {
...
exports.manuallyHostChannel = function(team, channel, callback) {
	checkIfChannelExistsAndOnline(channel, function(exists, name) {
...
exports.chooseChannel = function(team, channel) {
	...
	async.waterfall([
		function(callback) {
			if (!globalVars.isUsingManualChannelList[team]) {
				// Used to check if the team members have changed or not since the last host.
					...
					if (team) {
						...
						if (onlineChannels.length > 0) {
							...
							if (onlineChannels.length === 1) {
								...
								if (globalVars.preferredGames[team].length > 0 && !checkIfPlayingPreferredGame(team, onlineChannels[0])) {preferredGame = false;}
								chosenChannel = onlineChannels[0].username;
								loggingFuncs.logMessage(team, 'Only 1 channel online ' + (preferredGame?'(playing a preferred game)':'(not playing a preferred game)') + ', will host them.');
							}
							...
							// More than 1...
								...
								for (var i = 0; i < onlineChannels.length; i++) {
									if (checkIfPlayingPreferredGame(team, onlineChannels[i])) {
								...
								loggingFuncs.logMessage(team, 'Channels online ' + (preferredGame?'(playing a preferred game)':'(not playing a preferred game)') + ': ' + channelsToChoose.length + '.');
		...
		if (chosenChannel) {
			...
			var timeout = setTimeout(function() {exports.chooseChannel(team);}, refreshLength);
				...
				if (offlineChannels.length > 0) {
					...
					globalVars.client[team].on('notice', noticeEvent = function(channel, msgid, message) {
						if (channelsChecked.indexOf(channel) < 0) {
							...
							if (msgid === 'bad_host_rate_exceeded' || msgid === 'no_permission' || msgid === 'bad_host_hosting' || msgid === 'msg_banned') {
			...
			loggingFuncs.logMessage(team, 'No one to host, will check again in 5 minutes.', true);
			var timeout = setTimeout(function() {exports.chooseChannel(team);}, globalVars.recheckLength);
			globalVars.timeouts[team].push(timeout);
			globalVars.adminCommandsActive[team] = true;
		}
	});
...
function checkIfOffline(team) {
	...
	var timeout = setInterval(function() {
		twitchAPI.getStreamStatus([globalVars.currentHostedChannel[team].toLowerCase()], function(error, errorType, response) {
			// If there is no error and the stream is not reported as online, unhost them and move on.
			if (!error && !detectedOffline && response.streams.length === 0) {
				var justHostedChannel = globalVars.currentHostedChannel[team];
...
// Checks if the channel (object) passed to it is playing a preferred game (for that team) or not.
// If the team has no preffered games, this will always return false.
function checkIfPlayingPreferredGame(team, channel) {
	// Loops through all the preferred games to check them.
	for (var i = 0; i < globalVars.preferredGames[team].length; i++) {
		if (channel.currentGame && channel.currentGame.toLowerCase().indexOf(globalVars.preferredGames[team][i].toLowerCase()) >= 0) {
			return true;
		}
	}
...
function clearVariables(team, offline) {
