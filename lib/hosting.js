...
function chooseChannel(team) {
	getOnlineChannels(team, function(onlineChannels) {
		...
		if (chosenChannel) {
			...
			// Does the hosting stuff on all the channels.
			// Needs to check if the host was successful really before printing the message.
			// Also needs to check if the channel is offline before hosting there.
			for (var i = 0; i < globalVars.channels[team].length; i++) {
				globalVars.client[team].host(globalVars.channels[team][i], chosenChannel);
				globalVars.client[team].say(globalVars.channels[team][i], 'We have started hosting ' + chosenChannel + '.');
			}
		}
