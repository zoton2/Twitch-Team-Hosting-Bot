...
exports.turnOnHosting = function(team, callback) {
	if (!globalVars.active[team]) {
		...
		logging.logMessage(team, 'The hosting bot has been turned on.');
...
exports.turnOffHosting = function(team, callback) {
	if (globalVars.active[team]) {
		...
		// Stop monitoring for online channels if we are.
		if (monitoringForOnlineChannels[team]) {
			globalVars.streamEvents.removeListener('streamOnline', globalVars.monitorOnlineNotice[team]);
			...
			globalVars.client[team].on('unhost', unhostEvent = function(channel, viewers) {
				...
				if (channelsChecked.indexOf(channel) < 0) {
					...
					if (channelsChecked.length === globalVars.channels[team].length) {
						logging.logMessage(team, 'Successfully unhosted ' + lastHostedChannel.displayName + ' on all applicable channels.');
			...
			globalVars.client[team].on('notice', noticeEvent = function(channel, msgid, message) {
				if (channelsChecked.indexOf(channel) < 0) {
					...
					if (msgid === 'not_hosting' || msgid === 'no_permission' || msgid === 'msg_banned' || msgid === 'bad_host_error') {
						logging.logMessage(team, 'Could not unhost ' + lastHostedChannel.displayName + ' from ' + channel + ' (' + msgid + ').');
						channelsChecked.push(channel);
		...
		clearVariables(team);
		logging.logMessage(team, 'The hosting bot has been turned off.');
...
exports.manuallyHostChannel = function(team, channel, length, callback) {
	checkIfChannelExistsAndOnline(channel, function(exists, username, displayName, game, viewers) {
		if (!exists) {callback(false, 'That channel doesn\'t exist or is offline.');}
			...
			// Stop monitoring for online channels if we are (otherwise this can break the manual host).
			if (monitoringForOnlineChannels[team]) {
				globalVars.streamEvents.removeListener('streamOnline', globalVars.monitorOnlineNotice[team]);
				monitoringForOnlineChannels[team] = false;
			}
			...
			if (numOfTeamsChannelIsOn(username) === 0) {streamStatus.listenToChannels([username]);}
			
			var lengthInMS = (length && length >= 15) ? length*60000 : null;
			var preferredGame = checkIfPlayingPreferredGame(team, game);
			logging.logMessage(team, 'A manual host has been triggered for ' + displayName + (lengthInMS?' with a manual hosting length of '+length+'m':'') + ' (has ' + viewers + ' viewers).');
			if (globalVars.preferredGames[team].length > 0) {logging.logMessage(team, displayName + ' is ' + (preferredGame?'playing a preferred game.':'not playing a preferred game.'));}
...
exports.chooseChannel = function(team, channel, manualHostLength, preferredGame) {
	...
	async.waterfall([
		...
		function(callback) {
			...
			if (!chosenChannel) {
				...
				for (var i = 0; i < onlineChannels.length; i++) {
					if (utils.areWeHostingChannel(team, onlineChannels[i].username)	|| utils.areWeHostingChannel(team, onlineChannels[i].username, true)) {
				...
				if (onlineChannels.length > 0) {
					...
					if (onlineChannels.length === 1) {
						...
						logging.logMessage(team, chosenChannel.displayName + ' is the only channel online' + (globalVars.preferredGames[team].length>0?(preferredGame?' (playing a preferred game)':' (not playing a preferred game)'):'') + ', will host them (has ' + onlineChannels[0].currentViewers + ' viewers).');
					}
					...
					// More than 1...
						...
						logging.logMessage(team, 'Channels online' + (globalVars.preferredGames[team].length>0?(preferredGame?' (playing a preferred game)':' (not playing a preferred game)'):'') + ': ' + channelsToChoose.length + '.');
						logging.logMessage(team, 'Average viewers on those channels: ' + averageViewers + '.');
						logging.logMessage(team, 'Total viewers on those channels: ' + totalViewers + '.');
		...
		if (chosenChannel) {
			...
			if (globalVars.currentHostedChannel[team]) {
				logging.logMessage(team, 'Stopped hosting ' + globalVars.currentHostedChannel[team].displayName + ' (hosted for ' + exports.calculateHostedTime(team) + ').');
			}
			...
			getOfflineChannels(team, otherTeamChannels, function(offlineChannels) {
				// Remove channels that we don't want to host on right now from the array.
				offlineChannels = _.difference(offlineChannels, globalVars.channelsToNotHostOn[team]);
				
				globalVars.currentHostedChannel[team] = chosenChannel;
				globalVars.hostStartTime[team] = moment.utc();
				checkIfOffline(team);
				stats.incrementChannelStat(team, chosenChannel.username);
				...
				logging.logMessage(team, 'Started hosting ' + chosenChannel.displayName + ' (hosting length: ' + exports.formatMS(refreshLength) + ').');
				
				if (offlineChannels.length > 0) {
					var channelsChecked = [];
					var totalViewersForHost = 0;
					var hostEvent, noticeEvent;
					logging.logMessage(team, 'Channel(s) we will attempt to host on: ' + offlineChannels.join(', '));
					...
					globalVars.client[team].on('hosting', hostEvent = function(channel, target, viewers) {
						...
						if (channelsChecked.indexOf(channel) < 0 && offlineChannels.indexOf(channel) >= 0 && target.toLowerCase() === chosenChannel.username) {
					...
					globalVars.client[team].on('notice', noticeEvent = function(channel, msgid, message) {
						channel = channel.replace('#', '');
						if (channelsChecked.indexOf(channel) < 0 && offlineChannels.indexOf(channel) >= 0) {
							...
							if (msgid === 'bad_host_rate_exceeded' || msgid === 'no_permission' || msgid === 'bad_host_hosting' || msgid === 'msg_banned' || msgid === 'bad_host_error') {
								logging.logMessage(team, 'Could not host ' + chosenChannel.displayName + ' on ' + channel + ' (' + msgid + ').');
								checkIfChannelFullyHosted(team, channel, chosenChannel, hostEvent, noticeEvent, channelsChecked, offlineChannels, totalViewersForHost);
							}
						}
					});
					...
					async.whilst(
						...
						function(callback) {
							...
							i++; setTimeout(callback, 2000);
						}
					);
				}
				
				else {
					logging.logMessage(team, 'All channels currently online or do not want to be hosted on so ' + chosenChannel.displayName + ' has not been hosted anywhere.');
					globalVars.adminCommandsActive[team] = true;
				}
			});
		}
			...
			logging.logMessage(team, 'No one to host, will monitor for channels to go live.', true);
...
function checkIfChannelFullyHosted(team, channel, chosen, hostEvent, noticeEvent, channelsChecked, offline, viewersForHost) {
	...
	if (channelsChecked.length === offline.length) {
		logging.logMessage(team, 'Successfully hosted ' + chosen.displayName + ' for about ' + viewersForHost + ' viewers on all applicable channels.');
...
// Used to monitor for channels on the team to go live if we need to host someone else and no one else is currently on.
function monitorForOnlineChannels(team) {
	monitoringForOnlineChannels[team] = true;
	globalVars.streamEvents.on('streamOnline', globalVars.monitorOnlineNotice[team] = function(channel, streamInfo) {
		// If the channel that has come online is in this team, hosts them.
		if (globalVars.allTeamChannels[team].indexOf(channel) >= 0 && !utils.areWeHostingChannel(team, channel)) {
			globalVars.streamEvents.removeListener('streamOnline', globalVars.monitorOnlineNotice[team]);
			monitoringForOnlineChannels[team] = false;
			
			if (globalVars.adminCommandsActive[team]) {
				var preferredGame = checkIfPlayingPreferredGame(team, streamInfo.game);
				logging.logMessage(team, streamInfo.displayName + ' has come online' + (globalVars.preferredGames[team].length>0?(preferredGame?' (playing a preferred game)':' (not playing a preferred game)'):'') + ', will host them.');
				exports.chooseChannel(team, {username: streamInfo.username, displayName: streamInfo.displayName}, null, preferredGame);
			}
		}
	});
}

// Gets the online channels for the specified team.
function getOnlineChannels(team) {
	var onlineChannels = [];
	var channels = globalVars.teamChannels[team] || globalVars.allTeamChannels[team];
...
function getOfflineChannels(team, channels, callback) {
	...
	// Makes an array of all channels we think are offline.
	for (var i = 0; i < channels.length; i++) {
		if (!globalVars.liveStreamsInfo[channels[i]]) {allOfflineChannels.push(channels[i]);}
	}
	
	// Double check with the Twitch API in case we are listing a channel as offline when it's not.
	// (This was originally a temporary fix, but I decided to keep it in for safety.)
	twitchAPI.getStreamStatus(allOfflineChannels, function(error, errorType, response) {
		if (!error) {
			// If Twitch says a channel we think is offline is actually online, remove it from the array.
			for (var i = 0; i < response.streams.length; i++) {
				allOfflineChannels.splice(allOfflineChannels.indexOf(response.streams[i].channel.name), 1);
			}
			
			callback(allOfflineChannels);
		}
	});
}
...
function checkIfChannelExistsAndOnline(channel, callback) {
	...
	twitchAPI.getStreamStatus(channels, function(error, errorType, response) {
		if (error || response.streams.length === 0) {callback(false);}
			...
			var displayName = response.streams[0].channel.display_name || response.streams[0].channel.name;
...
function checkIfOffline(team) {
	...
	globalVars.streamEvents.on('streamOffline', globalVars.offlineNotice[team] = function(channel, streamInfo) {
		...
		if (globalVars.adminCommandsActive[team] && utils.areWeHostingChannel(team, channel)) {
			...
			var timeout = setTimeout(function() {
				...
				logging.logMessage(team, 'Stopped hosting ' + globalVars.currentHostedChannel[team].displayName + ' (went offline, hosted for ' + exports.calculateHostedTime(team) + ').');
				clearVariables(team);
			...
			globalVars.streamEvents.on('streamOnline', globalVars.onlineNotice[team] = function(channel_, streamInfo_) {
				...
				if (globalVars.adminCommandsActive[team] && utils.areWeHostingChannel(team, channel_)) {
...
function checkIfPlayingPreferredGame(team, game) {
	if (game) {
		...
		for (var i = 0; i < globalVars.preferredGames[team].length; i++) {
			if (game.toLowerCase().indexOf(globalVars.preferredGames[team][i].toLowerCase()) >= 0) {
				return true;
			}
		}
	}
...
// Used to check how many teams a specified channel is on.
// Obviously would only be 1 (or 0) if the bot wasn't set up for multiple teams.
function numOfTeamsChannelIsOn(channel) {
	var numOfTeams = 0;
...
// Used to check how many *other* teams are currently hosting the specified channel.
// This is used to check for a very rare bug that could happen, but might as well cover for it anyway!
function numOfOtherTeamsHostingChannel(teamToNotCheck, channel) {
	var numOfTeams = 0;
	
	// Loops through all the teams and checks if they are currently hosting the channel or not.
	for (var team in globalVars.currentHostedChannel) {
		if (globalVars.currentHostedChannel.hasOwnProperty(team)) {
			if (teamToNotCheck !== team
				&& globalVars.currentHostedChannel[team]
				&& globalVars.currentHostedChannel[team].username === channel) {
				numOfTeams++;
			}
		}
	}
...
function clearVariables(team) {
	...
	if (globalVars.currentHostedChannel[team] && numOfTeamsChannelIsOn(globalVars.currentHostedChannel[team].username) === 0) {
	...
	twitchAPI.getTeamChannels(team, function(error, errorType, response) {
		if (!error) {
			...
			if (newChannels.length > 0 || oldChannels.length > 0) {
				async.waterfall([
					// Starts listening to any new channels.
					function(callback2) {
						if (newChannels.length > 0) {streamStatus.listenToChannels(newChannels, callback2);}
						else {callback2();}
					},
					// Stops listening to any old channels.
					function(callback2) {
						if (oldChannels.length > 0) {
							// Only unlisten to channels that are on this team and not multiple,
							// and are not being hosted anywhere else currently (could be on a manual host?).
							var oldChansOnlyOn1Team = [];
							for (var i = 0; i < oldChannels.length; i++) {
								if (numOfTeamsChannelIsOn(oldChannels[i]) === 1
									&& numOfOtherTeamsHostingChannel(oldChannels[i]) === 0) {
									oldChansOnlyOn1Team.push(oldChannels[i]);
								}
							}
							
							streamStatus.unlistenToChannels(oldChansOnlyOn1Team, callback2);
						}
						else {callback2();}
					}
				], function(err) {
					...
					for (var i = 0; i < newChannels.length; i++) {
						...
						logging.logMessage(team, 'Channel added to team: ' + newChannels[i]);
					}
					...
					for (var i = 0; i < oldChannels.length; i++) {
						...
						logging.logMessage(team, 'Channel removed from team: ' + oldChannels[i]);
					}
