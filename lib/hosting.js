			...
			if (!chosenChannel) {
				...
				if (onlineChannels.length > 0) {
					...
					if (onlineChannels.length === 1) {
						...
						logging.logMessage(team, 'Only 1 channel online' + (globalVars.preferredGames[team].length>0?(preferredGame?' (playing a preferred game)':' (not playing a preferred game)'):'') + ', will host them (has ' + onlineChannels[0].currentViewers + ' viewers).');
					}
		...
		if (chosenChannel) {
			...
			getOfflineChannels(team, otherTeamChannels, function(offlineChannels) {
				...
				if (offlineChannels.length > 0) {
					...
					var hostEvent; globalVars.client[team].on('hosting', hostEvent = function(channel, target, viewers) {
					...
					// Listens for notices on all the channels.
					var noticeEvent; globalVars.client[team].on('notice', noticeEvent = function(channel, msgid, message) {
