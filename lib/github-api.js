			...
			if (lastLatestVersion !== latestVersion) {
				...
				for (var i = 0; i < 3; i++) {
					if (parseInt(latestVersionSplit[i]) > parseInt(currentVersionSplit[i])) {
						logging.logMessage(null, 'There is a new version available (current version: ' + globalVars.version + ' - latest version: ' + latestVersion + ').');
						globalVars.newVersion = latestVersion;
						break;
					}
					
					// If the current version is higher than the latest version, stops (only needed for testing newer versions).
					else if (parseInt(latestVersionSplit[i]) < parseInt(currentVersionSplit[i])) {break;}
				}
			}
		}
