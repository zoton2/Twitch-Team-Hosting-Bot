		...
		async.whilst(
			...
			function(callback) {
				...
				if (loginDetails[i].autoStart === undefined || loginDetails[i].autoStart === true) {
					...
					globalVars.active[loginDetails[i].team] = true;
				}
				
				else {globalVars.active[loginDetails[i].team] = false;}
				...
				twitchAPI.getTeamChannels(loginDetails[i].team, function(error, errorType, response) {
					if (!error) {
						...
						else {globalVars.channels[loginDetails[i].team] = loginDetails[i].manualChannelList;}
...
function connectToChat(team, account, oauth, callback) {
	...
	var ircOptions = {
		...
		channels: globalVars.channels[team]
	};
