	...
	if (!loginDetails) {
		...
		async.whilst(
			...
			function(callback) {
				...
				if (loginDetails[i].preferredGames && loginDetails[i].preferredGames.constructor === Array && loginDetails[i].preferredGames.length > 0) {globalVars.teamPreferredGames[loginDetails[i].team] = loginDetails[i].preferredGames;}
				else {globalVars.teamPreferredGames[loginDetails[i].team] = [];}
				...
				twitchAPI.getTeamChannels(loginDetails[i].team, function(error, errorType, response) {
					if (!error) {
						...
						else {globalVars.teamChannels[loginDetails[i].team] = loginDetails[i].manualChannelList;}
