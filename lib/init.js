...
function connectToChat(team, account, oauth, debug, callback) {
	...
	globalVars.client[team].on('join', initJoins = function(channel, username) {
		...
		if (channelCount === channels.length) {
			...
			connectWhispers(team, account, oauth, debug, function() {
				callback();
			});
		}
	});
...
// Called by the function above to also connect whispers.
function connectWhispers(team, account, oauth, debug, callback) {
	// Setting up the options.
	var ircOptions = {
		options: {
			debug: debug
	...
	// Will quit the application if whispers haven't connected after 1 minute.
	// (Sometimes it can't connect and never tries again for some reason.)
	var noConnectionTimeout = setTimeout(function() {
