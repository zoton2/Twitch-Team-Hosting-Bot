// Referencing packages.
var irc = require('tmi.js');
var async = require('async');
var fs = require('fs-extra');
var moment = require('moment');
var _ = require('underscore');

// Referencing other files.
var logging = require('./logging');
var globalVars = require('./global-vars');
var twitchAPI = require('./twitch-api');
var stats = require('./statistics');
var server = require('./server');
var utils = require('./utils');
var streamStatus = require('./stream-status');
var twitchPubSub = require('./twitch-pubsub');

exports.setUp = function(callback) {
	// Getting the settings from the Twitch API settings file the user should have (and should have changed).
	var twitchAPISettings; try {twitchAPISettings = fs.readJsonSync('persist/twitch-api-settings.json', {throws: false});} catch(e) {}
	
	// Stops the script from running if the file cannot be loaded.
	if (!twitchAPISettings) {
		logging.logMessage(null, 'There are issues loading the twitch-api-settings.json file.');
		process.exit(1);
	}
	
	// Check that things in the Twitch API settings file are correct.
	else {
		// Checks if the client ID has been set, and if so stores it.
		if (twitchAPISettings['clientID'] && _.isString(twitchAPISettings['clientID']) && twitchAPISettings['clientID'] != '' && twitchAPISettings['clientID'] != 'CLIENT_ID_GOES_HERE') {
			globalVars.clientID = twitchAPISettings['clientID'];
		}
		
		// If we believe the client ID hasn't been added, exits.
		else {
			logging.logMessage(null, 'You need to add your client ID to the twitch-api-settings.json file.');
			process.exit(1);
		}
	}
	
	// Getting the login details from the file the user should've made.	
	var loginDetails; try {loginDetails = fs.readJsonSync('persist/login-details.json', {throws: false});} catch(e) {}
	
	// Stops the script running if the file cannot be loaded.
	if (!loginDetails) {
		logging.logMessage(null, 'There are issues loading the login-details.json file.');
		process.exit(1);
	}
	
	// Check that things in the login details file are correct.
	else {
		if (loginDetails.length > 0) {
			var issues = [];
			
			// Checks all the login details for issues.
			for (var i = 0; i < loginDetails.length; i++) {
				if (!_.isString(loginDetails[i].team)) {issues.push('team');}
				if (!_.isString(loginDetails[i].username)) {issues.push('username');}
				if (!_.isString(loginDetails[i].oauth)) {issues.push('oauth');}
			}
			
			// If there are issues with required variables, exits.
			if (issues.length > 0) {
				logging.logMessage(null, 'There are ' + issues.length + ' issue(s) with your login-details.json file not containing required information.');
				process.exit(1);
			}
		}
		
		// If there are no details to use, exits.
		else {
			logging.logMessage(null, 'Your login-details.json file does not contain any settings or is not written correctly.');
			process.exit(1);
		}
	}
	
	// Getting the settings from the settings file, if it exists.
	try {globalVars.settings = fs.readJsonSync('persist/settings.json', {throws: false});} catch(e) {}
	
	// Tells the user if the file cannot be loaded, and sets defaults.
	if (!globalVars.settings) {
		logging.logMessage(null, 'There are issues loading the settings.json file, using defaults.');
		globalVars.settings = {};
	}
	
	// Check that things in the settings file are correct, and if not use defaults.
	else {
		if (!_.isBoolean(globalVars.settings.server)) {globalVars.settings.server = false;}
		if (!globalVars.settings.serverPort) {globalVars.settings.serverPort = 8080;}
		if (!_.isBoolean(globalVars.settings.logServerAccess)) {globalVars.settings.logServerAccess = true;}
	}
	
	// Loading the statistics...
	stats.loadStatistics(function() {
		// Start the server if the user wants it.
		if (globalVars.settings.server) {server.setUp();}
		
		var autoStartList = [];
		
		// Goes through each team and gets the details needed.
		var i = 0;
		async.whilst(
			function() {return i < loginDetails.length;},
			function(callback) {
				var team = loginDetails[i].team;
				if (loginDetails[i].teamChannels && _.isArray(loginDetails[i].teamChannels) && loginDetails[i].teamChannels.length > 0) {
					team = 'custom-' + loginDetails[i].team;
					globalVars.teamChannels[team] = utils.lowercaseArray(loginDetails[i].teamChannels);
				}
				
				// Sets up default variables for this team.
				globalVars.mainChannel[team] = (_.isString(loginDetails[i].mainChannel)) ? loginDetails[i].mainChannel : loginDetails[i].username;
				globalVars.preferredGameHostLength[team] = (!isNaN(loginDetails[i].preferredGameHostLength)) ? loginDetails[i].preferredGameHostLength*60000 : 7200000;
				globalVars.nonPreferredGameHostLength[team] = (!isNaN(loginDetails[i].nonPreferredGameHostLength)) ? loginDetails[i].nonPreferredGameHostLength*60000 : 1800000;
				globalVars.hostTrainMessage[team] = (_.isString(loginDetails[i].hostTrainMessage)) ? loginDetails[i].hostTrainMessage : null;
				globalVars.modsAreAdmins[team] = (_.isBoolean(loginDetails[i].modsAreAdmins)) ? loginDetails[i].modsAreAdmins : false;
				globalVars.channels[team] = [];
				globalVars.currentHostedChannel[team] = null;
				globalVars.active[team] = false;
				globalVars.timeouts[team] = [];
				globalVars.adminCommandsActive[team] = false;
				globalVars.channelsToNotHostOn[team] = [];
				globalVars.oauthToken[team] = loginDetails[i].oauth.replace('oauth:', '');
				
				// Sets the list of preferred games if set, otherwise sets it to blank.
				if (loginDetails[i].preferredGames && loginDetails[i].preferredGames.constructor === Array && loginDetails[i].preferredGames.length > 0) {globalVars.preferredGames[team] = loginDetails[i].preferredGames;}
				else {globalVars.preferredGames[team] = [];}
				
				// If the settings want this team to start hosting as soon as the bot starts (or omits the setting) sets this.
				if (loginDetails[i].autoStart === undefined || loginDetails[i].autoStart === true) {
					autoStartList.push(team);
				}
				
				// Sets the list of admins if set, otherwise sets it to blank.
				if (loginDetails[i].admins && loginDetails[i].admins.constructor === Array && loginDetails[i].admins.length > 0) {globalVars.admins[team] = loginDetails[i].admins;}
				else {globalVars.admins[team] = [];}
				
				// Converts admin names to lower case (for easier comparison elsewhere in the code).
				globalVars.admins[team] = utils.lowercaseArray(globalVars.admins[team]);
				
				// Checks if the debug setting specified is a boolean, and sets it to false if not.
				if (typeof(loginDetails[i].debug) != "boolean") {loginDetails[i].debug = false;}
				
				async.waterfall([
					function(callback2) {
						globalVars.teamLastCheck[team] = moment().unix();
						
						// Queries the API for a list of channels on the team if needed.
						if (!loginDetails[i].manualChannelList || !_.isArray(loginDetails[i].manualChannelList) || loginDetails[i].manualChannelList.length === 0) {
							globalVars.isUsingManualChannelList[team] = false;
							
							if (!globalVars.teamChannels[team]) {
								logging.logMessage(team, 'Getting a list of current team channels from Twitch.');
								twitchAPI.getTeamChannels(team, function(error, errorType, response) {
									if (!error) {
										// Pushes all of the channels on the team into the correct array.
										for (var j = 0; j < response.length; j++) {
											globalVars.channels[team].push(response[j].username);
										}
										
										globalVars.allTeamChannels[team] = globalVars.channels[team];
										callback2();
									}
								});
							}
							
							else {globalVars.channels[team] = globalVars.teamChannels[team]; callback2();}
						}
						
						// If a manual channel list has been specified, uses that instead.
						else {
							globalVars.isUsingManualChannelList[team] = true;
							loginDetails[i].manualChannelList = utils.lowercaseArray(loginDetails[i].manualChannelList);
							globalVars.channels[team] = loginDetails[i].manualChannelList;
							
							if (!globalVars.teamChannels[team]) {
								logging.logMessage(team, 'Getting a list of current team channels from Twitch.');
								twitchAPI.getTeamChannels(team, function(error, errorType, response) {
									if (!error) {
										var currentTeamChannels = [];
										for (var j = 0; j < response.length; j++) {
											currentTeamChannels.push(response[j].username);
										}
										
										globalVars.allTeamChannels[team] = currentTeamChannels;
										streamStatus.listenToChannels(currentTeamChannels, callback2);
									}
								});
							}
							
							else {streamStatus.listenToChannels(globalVars.teamChannels[team], callback2);}
						}
					},
					function(callback2) {
						streamStatus.listenToChannels(globalVars.channels[team], callback2);
					},
					function(callback2) {
						// Connects to the chat using the bot account provided.
						connectToChat(team, loginDetails[i].username, loginDetails[i].oauth, loginDetails[i].debug, function() {
							// Waiting for 2 seconds before finishing set up to make sure all hosting events have come through.
							setTimeout(function() {
								streamStatus.setUpHostChecking(team);
								logging.logMessage(team, 'Finished initial setting up and connecting.');
								callback2();
							}, 2000);
						});
					}
				], function(err) {i++; callback()});
			},
			function(err) {
				// We are done connecting the accounts.
				callback(autoStartList);
			}
		);
	});
}

// Used by the above function to connect bots to the chat.
function connectToChat(team, account, oauth, debug, callback) {
	var channels = globalVars.channels[team].slice(0);
	
	// Joins the main channel as well if it isn't specified anywhere else.
	if (channels.indexOf(globalVars.mainChannel[team].toLowerCase()) === -1) {
		channels.push(globalVars.mainChannel[team].toLowerCase());
	}
	
	// Setting up the options.
	var ircOptions = {
		options: {
			debug: debug
		},
		connection: {
			reconnect: true,
			secure: true
		},
		identity: {
			username: account,
			password: oauth
		},
		channels: channels
	};
	
	// Sets up the client.
	globalVars.client[team] = new irc.client(ircOptions);
	globalVars.client[team].connect();
	
	// Will quit the application if we haven't connected after 1 minute.
	// (Sometimes it can't connect and never tries again for some reason.)
	var noConnectionTimeout = setTimeout(function() {
		logging.logMessage(team, 'Cannot connect; quitting. Try running the application again.');
		process.exit(1);
	}, 60000);
	
	// Clears the timeout above once the connection is done.
	globalVars.client[team].once('connected', function(address, port) {
		clearTimeout(noConnectionTimeout);
	});
	
	// Fires the call back once all of the channels are joined.
	var channelCount = 0;
	var initJoins;
	globalVars.client[team].on('join', initJoins = function(channel, username, self) {
		if (self) {channelCount++;}
		
		if (channelCount === channels.length) {
			globalVars.client[team].removeListener('join', initJoins);
			callback();
		}
	});
	
	// Logging for connecting event.
	globalVars.client[team].on('connecting', function(address, port) {
		logging.logMessage(team, 'Connecting to chat (' + address + ':' + port + ')');
	});
	
	// Logging for connected event.
	globalVars.client[team].on('connected', function(address, port) {
		logging.logMessage(team, 'Connected to chat (' + address + ':' + port + ')');
	});
	
	// Logging for disconnected event.
	globalVars.client[team].on('disconnected', function(reason) {
		logging.logMessage(team, 'Connection to chat has disconnected: ' + reason);
	});
	
	// Logging for reconnect event.
	globalVars.client[team].on('reconnect', function() {
		logging.logMessage(team, 'Connection to chat is reconnecting.');
	});
}

// Listens to see if something has asked this application to close.
// Could also try to unhost but that's not always wanted, seeing as it
// could waste 1 of your 3 hosts you can do every 30 minutes.
process.on('SIGINT', function() {
	logging.logMessage(null, 'Application is going to close.');
	
	// Disconnect the chat connections for all teams.
	for (var team in globalVars.client) {
		if (globalVars.client.hasOwnProperty(team)) {
			globalVars.client[team].disconnect();
		}
	}
	
	twitchPubSub.disconnectAll();
	
	// Waits a second to make sure stuff is finished before exiting.
	setTimeout(function() {
		// Delay added to make sure the next log message actually gets saved to the file.
		logging.logMessage(null, 'Application is now closing.');
		setTimeout(function() {process.exit(0);}, 200);
	}, 1000);
});